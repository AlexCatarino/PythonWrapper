...
namespace QuantConnect.PythonWrapper
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            AppendWrapperMethods();
            AppendWrapperProperties();

            Console.WriteLine(_wrapperContent.ToString());
            File.WriteAllText("AutoGen.cs", _wrapperContent.ToString());
            Console.WriteLine("AutoGen.cs was created.");

            //_templateContent = new StringBuilder();
            //AppendTemplateMethods();

            //Console.WriteLine(_templateContent.ToString());
            //File.WriteAllText("QCPyAlgorithmMaster.py", _templateContent.ToString());
            //Console.WriteLine("QCPyAlgorithmMaster.py was created.");
        ...
        private static void AppendWrapperProperties()
        {
            var properties = _type.GetProperties().OrderBy(x => x.Name).Where(x => x.DeclaringType == typeof(QCAlgorithm));
        ...
        private static void AppendWrapperMethods()
        {
            var methods = _type.GetMethods().OrderBy(x => x.Name).Where(x => x.DeclaringType == typeof(QCAlgorithm) && !x.Name.Contains("et_"));
